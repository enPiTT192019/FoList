    ...
    fun seekbarDialog(
        ...
        val view = (context.getSystemService(Context.LAYOUT_INFLATER_SERVICE) as LayoutInflater)
            .inflate(R.layout.dialog_seek_bar, null).apply {
                ...
                progressSeekBar.setOnSeekBarChangeListener(object :
            ...
            .inflate(R.layout.dialog_seek_bar, null).apply {
                ...
                fun refresh(i: Int) {
                    if (i < questions.size && i<otherAnswers.size &&i<correctAnswers.size) {
                        ...
                        textViews[correctAnswer].text=correctAnswers[i]
                        var temp=0
                        textViews.forEachIndexed {index,textView->
                            if(index!=correctAnswer){
                                textView.text=otherAnswers[i][temp++]
                            }
...
//                    addChild(RawTreeNode(NodeValue("生きる"),this,mRealm=realm))
                ...
                addChild(RawTreeNode(NodeValue("基本情報資格を取得"), this, mRealm = realm).apply {
                    ...
                    addChild(
                        RawTreeNode(
                            NodeValue("二ヶ月毎日練習問題を読む", power = 60),
                            this,
                            mRealm = realm
                        )
                    )
                ...
                addChild(RawTreeNode(NodeValue("『TOEIC単語マスター』"), this, mRealm = realm).apply {
                    //TODO:バーコード連携のアピール
                    addChild(RawTreeNode(NodeValue("Chapter1"), this, mRealm = realm).apply {
                        value!!.type = NodeTypes.TEST_NODE.name
                        addChild(RawTreeNode(NodeValue("hello"), this, mRealm = realm))
